...
describe('agent:skill:index', () => {
  ...
  beforeEach(async () => {
    ...
    const imported = await esmock(
      '../../../../src/commands/claude/skill/index.js',
      {},
      {
    ...
    SkillList = imported.default
    SkillList.id = 'claude:skill'
  })

  it('re-registers dynamic capability commands against the reloaded config, then renders the topic overview', async () => {
    const cmd = new SkillList([], commandOptions as any)

    await cmd.run()

    expect(registerCapabilityCommandsStub.calledOnceWith(cmd.config, 'claude:skill')).to.be.true
    expect(renderTopicHelpStub.calledOnceWith(cmd.config, 'claude:skill')).to.be.true
