...
describe('agent:auth:test', () => {
  ...
  beforeEach(async () => {
    ...
    const imported = await esmock('../../../../src/commands/claude/auth/test.js', {
  ...
  it('shows success on valid connection', async () => {
    ...
    expect(testConnectionStub.calledWith(mockAuth)).to.be.true
    expect(clearClientsStub.calledOnce).to.be.true
    expect(actionStopStub.calledWith('✓ successful')).to.be.true
    expect(logStub.calledWith('Successful connection to AI provider')).to.be.true
  ...
  it('shows error on failed connection', async () => {
    ...
    expect(errorStub.calledWith('Failed to connect to AI provider.')).to.be.true
  })
  ...
  it('passes --profile to readAgentConfig', async () => {
    testConnectionStub.resolves({success: true})

    const cmd = new AgentAuthTest(['--profile', 'work'], {
      configDir: '/tmp/test-agent-config',
      root: process.cwd(),
      runHook: stub().resolves({failures: [], successes: []}),
    } as any)
